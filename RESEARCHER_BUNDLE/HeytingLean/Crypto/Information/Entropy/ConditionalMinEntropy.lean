...
We treat conditional min-entropy as an abstract quantity `H_min(X|E)` with a single
axiom: conditioning does not increase entropy.
-/
...
structure ConditionalMinEntropy (X E : Type*) [MinEntropySpace X] where
  ...
  /-- Conditioning decreases entropy (interface axiom). -/
