...
We provide:
- finite/discrete definitions of statistical distance and uniform distributions;
  ...
  lhl :
      ∀ (X : D → ℝ),
        ...
        ∀ (k : ℝ),
          ...
          Finite.statDistance (Finite.jointDist (R := R) (H := (TwoUniversal.toHashFamily (D := D) (R := R) (S := S))) X)
